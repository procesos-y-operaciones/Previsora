...
class TypeProcess < ApplicationRecord
  ...
  def self.total_headers
    [
      ...
      'HAY IMPUGNACION?',	'HAY DESACATO?', 'OTRO DESPACHO', 'ZONA', 'ULTIMA ACTUALIZACION', 'FECHA DE CREACION','OBSERVACIONES'
    ]
  end
  ...
  def get_total_content
    [
      ...
      booleanValue(self.has_more_polcies), booleanValue(self.has_costs), booleanValue(self.has_coactive_radicate), booleanValue(self.has_impug), booleanValue(self.has_desacate), self.other_office_name, self.get_zone, (self.updated_at-5.hour).strftime("%d/%m/%Y %H:%M"), (self.created_at-5.hour).strftime("%d/%m/%Y %H:%M"),self.observations
    ]
  end
