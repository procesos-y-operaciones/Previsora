class CoordinatorController < ApplicationController
  ...
  def report
    ...
    respond_to do |format|
      ...
      format.xls { response.headers['Content-Disposition'] = "attachment; filename=#{Date.today.to_s}.xlsx" }#{ send_data @report.to_csv(@date_from, @date_till, col_sep: "\t"), filename: Date.today.to_s+'.xls' }
    end
  end
