class ApplicationController < ActionController::Base
  ...
  def offices
    ...
    render json: OfficeName.select(:id, :name).order(name: :asc).where(
      "name like ? and name like ? or name like ? and name like ? or name like ? and name like ?",
      "%#{@parameters[0]}%", "%#{@parameters[3]}%", "%#{@parameters[1]}%", "%#{@parameters[3]}%", "%#{@parameters[2]}%", "%#{@parameters[3]}%",
    ).to_json
  end
